...
*/
    ...
    private:
        ...
        inline void storeOBJ (const std::string& filename);
        inline void storePLY (const std::string& filename);
    ...
    void Mesh::store (const std::string & filename) {
        ...
        if (formatString == "off")
            ...
            storeOBJ(filename);
        else if (formatString == "ply")
            storePLY(filename);
        else
            throw MorphoGraphics::Mesh::Exception("Error storing mesh: unrecognized file format.");
    }
    ...
    void Mesh::storeOBJ(const std::string& filename) {
        std::ofstream out(filename.c_str());
        if (!out) {
            throw Exception("Error storing OBJ file: " + filename);
        }

        out << "# generated by Broxy - " << _P.size() << " vertices - " << _T.size() << " faces" << std::endl;
        ...
        // Write faces
        for (const auto &i: _T) {
            out << "f " << i[0] + 1 << " " << i[1] + 1 << " " << i[2] + 1 << std::endl;
        }

        out.close();
    }

    void Mesh::storePLY(const std::string& filename) {
        std::ofstream out(filename.c_str());
        if (!out) {
            throw Exception("Error storing PLY file: " + filename);
        }

        // Write header
        out << "ply" << std::endl;
        out << "format ascii 1.0" << std::endl;
        out << "comment generated by Broxy" << std::endl;
        out << "element vertex " << _P.size() << std::endl;
        out << "property float x" << std::endl;
        out << "property float y" << std::endl;
        out << "property float z" << std::endl;
        out << "element face " << _T.size() << std::endl;
