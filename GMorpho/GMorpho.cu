...
void GMorpho::Load3DGS (const std::string& path_3dgs_folder, int base_res, float se_size, float threshold, bool allow_cache, bool use_tsdf) {
	se_size_ = se_size;
	...
	gsvoxelizer_.Load (path_3dgs_folder);
	if (use_tsdf) {
		gsvoxelizer_.VoxelizeConservativeTSDF (base_res, margin);
	} else {
		gsvoxelizer_.VoxelizeConservative (base_res, margin, threshold, allow_cache);
	}
