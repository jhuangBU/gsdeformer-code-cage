...
/**
 * @remarks populate compute YZ bbox with v0/v1/v2
 */
__device__ 
...
void Voxelizer::VoxelizeConservative (int base_res, float margin) {
	...
	std::vector<unsigned int> buck_vox_grid (res_x * res_y * res_z, 0); // 32b per entry, could hold 4*8 points, or 4 points
	glBindTexture (GL_TEXTURE_3D, vox_tex_);
	MorphoGraphics::GL::printOpenGLError ("Binding voxel texture");
	int get_tex_level = 0;
	...
	std::vector<unsigned char> pack_vox_grid; // 8b per entry, 1b per points, 8 points
		...
		for (int j = 0; j < res_y; j++)
			for (int k = 0; k < res_z; k++) {
				...
				if (value != 0)	{
					if (num_bit_vox == 1)
						for (int s = 0; s < 32; s++) {
                            // if there is value in it
							if ((value & (0x00000001)) == 0x00000001) {
								Vec3i position (i, j, 32 * k + s);
                                // because we pack 8 vertices, xyz, x(+1)y(+1)z(+1) all into one?
