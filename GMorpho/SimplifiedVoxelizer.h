...
#ifndef  SIMPL_VOXELIZER_INC
#define  SIMPL_VOXELIZER_INC
...
class SimplifiedVoxelizer {
public:

    // lifecycles
	SimplifiedVoxelizer ();
	virtual ~SimplifiedVoxelizer ();
	...
	void Load (const float * P, const float * N, int num_of_vertices,
	           const unsigned int * T, int num_of_faces); // checked: public
    // compute:voxelize
	void VoxelizeConservative (int base_res, float margin); // checked: public
private:
    void ComputeGridAttributes (int base_res, float margin); // checked: private
    // visual:rendering & OpenGL
    void LoadShaders (); // checked: private
	void BuildFBO (int res, int num_bit_vox); // checked: private
	void ClearFBO (); // checked: private
    // compute:voxelize
	void VoxelizeSurfaceConservative (GL::Program * surf_vox_conserv_program); // checked: private
	void VoxelizeVolume (GL::Program * vol_vox_program); // checked: private

	void ConvertBucketToPacketGrid (const std::vector<unsigned int> & buck_vox_grid,
	                                int res, int num_bit_vox,
	                                std::vector<unsigned char> & pack_vox_grid); // checked: private
	void ParseGrid (const std::vector<unsigned int> & vox_grid,
	                const AxisAlignedBoundingBox & bbox,
	                int res, int num_bit_vox,
	                std::vector<Vec3f> & positions,
	                std::vector<Vec3f> & normals); // checked: private (commented)
...
#endif   /* ----- #ifndef SIMPL_VOXELIZER_INC  ----- */
