...
class DecimationParamsDialog : public QDialog {
Q_OBJECT

public:
    DecimationParamsDialog(DecimationParams& params, QWidget* parent = nullptr)
            : QDialog(parent), m_params(params) {
        setWindowTitle("Decimation Parameters");

        QFormLayout* layout = new QFormLayout(this);

        m_targetErrorEdit = new QLineEdit(QString::number(params.target_error));
        m_targetErrorEdit->setValidator(new QDoubleValidator(0, 100, 2, this));
        layout->addRow("Target Error:", m_targetErrorEdit);

        m_targetNumFacesEdit = new QLineEdit(QString::number(params.target_num_faces));
        m_targetNumFacesEdit->setValidator(new QIntValidator(1, 1000000, this));
        layout->addRow("Target Number of Faces:", m_targetNumFacesEdit);

        m_targetNumVerticesEdit = new QLineEdit(QString::number(params.target_num_vertices));
        m_targetNumVerticesEdit->setValidator(new QIntValidator(1, 1000000, this));
        layout->addRow("Target Number of Vertices:", m_targetNumVerticesEdit);

        m_maxEdgeLengthAlphaEdit = new QLineEdit(QString::number(params.max_edge_length_alpha));
        m_maxEdgeLengthAlphaEdit->setValidator(new QDoubleValidator(0, 100, 2, this));
        layout->addRow("Max Edge Length Alpha:", m_maxEdgeLengthAlphaEdit);

        QPushButton* okButton = new QPushButton("OK");
        QPushButton* cancelButton = new QPushButton("Cancel");
        connect(okButton, &QPushButton::clicked, this, &DecimationParamsDialog::accept);
        connect(cancelButton, &QPushButton::clicked, this, &DecimationParamsDialog::reject);
    ...
    void accept() override {
        m_params.target_error = m_targetErrorEdit->text().toFloat();
        m_params.target_num_faces = m_targetNumFacesEdit->text().toInt();
        m_params.target_num_vertices = m_targetNumVerticesEdit->text().toInt();
        m_params.max_edge_length_alpha = m_maxEdgeLengthAlphaEdit->text().toFloat();
        QDialog::accept();
    }

private:
    DecimationParams& m_params;
    QLineEdit* m_targetErrorEdit;
    QLineEdit* m_targetNumFacesEdit;
    QLineEdit* m_targetNumVerticesEdit;
    QLineEdit* m_maxEdgeLengthAlphaEdit;
};
...
public slots:
    ...
    void load3DGS (const QString & dirpath, float density = 1e-6, bool use_tsdf = true);
	void storeProxy (const QString & filename);
	void runInterleavedDecimation ();
    // configs
...
private:
	...
	std::string _gaussianPlyPath;  // resolved 3DGS PLY path (empty when mesh-only)
};
