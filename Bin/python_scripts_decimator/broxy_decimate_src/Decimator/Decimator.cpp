...
class CQEMEdgeCollapse : public vcg::tri::TriEdgeCollapseQuadric<MyMesh, VertexPair, CQEMEdgeCollapse, QInfoStandard<
            MyVertex> > {
public:
    ...
    CoordType ComputeMinimal(BaseParameterClass *_pp, bool use_linear_constraints, bool &is_feasible, bool &is_optimal,
                             bool &is_bounded, bool &rt);

    CoordType ComputeMinimal(BaseParameterClass *_pp, bool use_linear_constraints, bool &is_feasible, bool &is_optimal,
                             bool &is_bounded, bool &rt, ScalarType &objective);
...
bool CQEMEdgeCollapse::IsFeasible(BaseParameterClass *_pp) {
    ...
    if (pp->NormalCheck) {
        // Compute maximal normal variation
    ...
    float max_edge_length_alpha = ((MyTriEdgeCollapseQuadricParameter *) pp)->max_edge_length_;
...
ScalarType CQEMEdgeCollapse::ComputePriority(BaseParameterClass *_pp,
    ...
    ScalarType objective;
    if (pp->OptimalPlacement) {
        c_qem_x = ComputeMinimal(pp, ((MyTriEdgeCollapseQuadricParameter *) pp)->use_linear_constraints_, is_feasible,
                                 is_optimal, is_bounded, rt, objective);
        v[0]->P() = c_qem_x;
        v[1]->P() = v[0]->P();
    } else {
        v[0]->P() = v[1]->P();
...
CoordType CQEMEdgeCollapse::ComputeMinimal(BaseParameterClass *_pp, bool use_linear_constraints,
    ...
    ScalarType obj;
    CoordType x = ComputeMinimal(_pp, use_linear_constraints, is_feasible, is_optimal, is_bounded, rt, obj,
                                 print_debug);
    return x;
}
...
CoordType CQEMEdgeCollapse::ComputeMinimal(BaseParameterClass *_pp, bool use_linear_constraints,
                                           ...
                                           ScalarType &objective) {
    bool print_debug = false;
    CoordType x = ComputeMinimal(_pp, use_linear_constraints, is_feasible, is_optimal, is_bounded, rt, objective,
                                 print_debug);
    return x;
}
...
void CQEMEdgeCollapse::SolveQP(Eigen::Matrix3d &c, Eigen::Vector3d &d, double &d0, Eigen::Matrix3d &omega,
    ...
    v[0] = this->pos.V(0);
    v[1] = this->pos.V(1);
    RunSolveQP(c, d, omega, sol, use_linear_constraints, is_feasible, is_optimal, is_bounded, qp_ws, v);
}
            ...
            if ((*pf).V(0)->IsR() && (*pf).V(1)->IsR() && (*pf).V(2)->IsR()) {
                ...
                for (j = 0; j < 3; ++j)
                    if ((*pf).IsB(j) || pp->QualityQuadric) // Bordo!
                    {
                        ...
                        if ((*pf).IsB(j)) pb.SetDirection(pb.Direction() * (ScalarType) pp->BoundaryWeight);
                            // amplify border planes
                        else pb.SetDirection(pb.Direction() * (ScalarType) (pp->BoundaryWeight / 100.0));
                        // and consider much less quadric for quality
            ...
            if (!(*vi).IsD() && (*vi).IsRW()) {
                ...
                for (x.F() = (*vi).VFp(), x.I() = (*vi).VFi(); x.F() != 0; ++x) {
                    ...
                    if ((x.V0() < x.V1()) && x.V1()->IsRW() && !x.V1()->IsV()) {
                        x.V1()->SetV();
                        h_ret.push_back(HeapElem(new CQEMEdgeCollapse(VertexPair(x.V0(), x.V1()),
                                                                      TriEdgeCollapse<MyMesh, VertexPair,
                                                                          CQEMEdgeCollapse>::GlobalMark(), _pp)));
                    }
            ...
            if (!(*vi).IsD() && (*vi).IsRW()) {
                ...
                for (x.F() = (*vi).VFp(), x.I() = (*vi).VFi(); x.F() != 0; ++x) {
                    ...
                    if (x.V()->IsRW() && x.V1()->IsRW() && !IsMarked(m, x.F()->V1(x.I()))) {
                        h_ret.push_back(HeapElem(new CQEMEdgeCollapse(VertexPair(x.V(), x.V1()),
                                                                      TriEdgeCollapse<MyMesh, VertexPair,
                                                                          CQEMEdgeCollapse>::GlobalMark(), _pp)));
                    }
                    if (x.V()->IsRW() && x.V2()->IsRW() && !IsMarked(m, x.F()->V2(x.I()))) {
                        h_ret.push_back(HeapElem(new CQEMEdgeCollapse(VertexPair(x.V(), x.V2()),
                                                                      TriEdgeCollapse<MyMesh, VertexPair,
                                                                          CQEMEdgeCollapse>::GlobalMark(), _pp)));
                    }
                }
...
VCGOptimSession::VCGOptimSession(const MorphoGraphics::Mesh &mesh,
    ...
    deci_session_->SetTargetOperations(1);
    ...
    num_vertices_ = decimated_mesh_.vn;
    error_ = 0.f;

    //	delete deci_session_;
}

VCGOptimSession::~VCGOptimSession() { delete qparams.qp_ws_; };

bool VCGOptimSession::Optimize() {
...
void VCGOptimSession::GetMesh(MorphoGraphics::Mesh &output_mesh) {
...
bool VCGOptimSession::UpdateVertexPositions(const MorphoGraphics::Mesh &mesh) {
    const std::vector<MorphoGraphics::Vec3f> &p = mesh.P();
    const std::vector<MorphoGraphics::Vec3f> &n = mesh.N();

    if ((int)p.size() != decimated_mesh_.vn) {
        return false;
    }
    ...
    int i = 0;
    for (VertexIterator vi = decimated_mesh_.vert.begin(); vi != decimated_mesh_.vert.end(); ++vi) {
        if ((*vi).IsD()) {
            continue;
        }
        (*vi).P() = CoordType(p[i][0], p[i][1], p[i][2]);
    ...
    if (n.empty()) {
        tri::UpdateNormal<MyMesh>::PerVertexPerFace(decimated_mesh_);
        tri::UpdateNormal<MyMesh>::NormalizePerVertex(decimated_mesh_);
    }
    tri::UpdateBounding<MyMesh>::Box(decimated_mesh_);

    for (auto &elem : deci_session_->h) {
        delete elem.locModPtr;
    }
    deci_session_->h.clear();

    deci_session_->Init<CQEMEdgeCollapse>();
    deci_session_->SetTargetSimplices(target_num_faces_);
    deci_session_->SetTimeBudget(0.5f);
    if (target_error_ < std::numeric_limits<float>::max()) {
        deci_session_->SetTargetMetric(target_error_);
    }
