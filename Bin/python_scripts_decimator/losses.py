...
class OneSidedChamferDistance(torch.nn.Module):
    """
    One-sided proximity loss: cage -> shape only.
    Computes mean squared distance from each point in `xyz1` to its nearest
    neighbor in `xyz2` using FAISS KNN (k=1).

    Args:
        None

    Forward:
        xyz1: (B, N, 3) query points (e.g., cage vertices)
        xyz2: (B, M, 3) support points (e.g., source mesh vertices)

    Returns:
        loss: scalar tensor (mean over all points)
        idx12: (B, N, 1) indices in `xyz2` of nearest neighbors for each query
    """
    def __init__(self):
        super().__init__()

    def forward(self, xyz1: torch.Tensor, xyz2: torch.Tensor):
        # Ensure contiguous tensors in (B, N, 3)
        xyz1 = xyz1.contiguous()
        xyz2 = xyz2.contiguous()
        # Prefer a pure-torch KNN to avoid FAISS version issues.
        # group_knn returns squared distances D >= 0 as the 3rd output.
        _, idx12, d2 = group_knn(1, xyz1, xyz2, unique=False, NCHW=False)
        # (B, N, 1) squared distances; take mean
        loss = d2.mean()
        return loss, idx12
