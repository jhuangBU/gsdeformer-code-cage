...
def render(
    ...
    if cov3d is not None:
        assert cov3d.shape == (means3D.shape[0], 6), "custom cov3d must be in stripped symmetric format!"
        cov3D_precomp = cov3d
    else:
    ...
    rendered_image, radii, depth = rasterizer(
        ...
        scales=scales.float() if scales is not None else scales,
        rotations=rotations.float() if rotations is not None else rotations,
