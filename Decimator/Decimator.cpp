...
void Decimator::LoadPLY (const std::string & filename, MorphoGraphics::Mesh & output_mesh) {
	MyMesh vcg_mesh;
	int mask = 0;
	int result = vcg::tri::io::ImporterPLY<MyMesh>::Open(vcg_mesh, filename.c_str(), mask);
	if (result != 0)
		throw std::runtime_error("Decimator::LoadPLY: failed to load " + filename);

	std::vector<MorphoGraphics::Vec3f> & output_p = output_mesh.P();
	SimpleTempData<typename MyMesh::VertContainer, int> indices(vcg_mesh.vert);

	int i;
	VertexIterator vi;
	FaceIterator fi;
	output_p.resize(vcg_mesh.vn);
	VertexPointer vp;
	for (i = 0, vi = vcg_mesh.vert.begin(); vi != vcg_mesh.vert.end();) {
		vp = &(*vi);
		indices[vi] = i;
		if (!vp->IsD()) {
			output_p[i][0] = vp->P()[0];
	...
	std::vector<MorphoGraphics::Vec3<unsigned int>> & output_t = output_mesh.T();
	FacePointer fp;
	output_t.resize(vcg_mesh.fn);
	for (i = 0, fi = vcg_mesh.face.begin(); fi != vcg_mesh.face.end();) {
		fp = &(*fi);
		if (!fp->IsD()) {
			output_t[i][0] = indices[fp->cV(0)];
			output_t[i][1] = indices[fp->cV(1)];
...
Decimator::Decimator (const MorphoGraphics::Mesh & mesh, int target_num_faces, 
    ...
    num_vertices_ = vcg_optim_session_->num_vertices();
	target_num_faces_ = target_num_faces;
